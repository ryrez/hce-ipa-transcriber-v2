...
def initialize_sheets():
    ...
    if credentials:
        try:
            ...
            # Try to open/create the learning spreadsheet
            try:
                sheet = client.open("HCE_IPA_Learning_Data").sheet1
            except:
                # Create new spreadsheet if it doesn't exist
                spreadsheet = client.create("HCE_IPA_Learning_Data")
                sheet = spreadsheet.sheet1
                # Set up headers
                sheet.update('A1:H1', [['Timestamp', 'Word', 'Original', 'IPA_Choice', 'Interaction_Type', 'Confidence', 'Selection_Count', 'Session_ID']])
            
            return sheet
        except ImportError:
            st.sidebar.warning("Install gspread and google-auth packages for Google Sheets integration")
            return None
        except Exception as e:
...
# --- Streamlit UI Setup ---
st.set_page_config(
    page_title="HCE IPA Transcriber with Google Sheets", 
    layout="wide",
...
if "sheets_connection" not in st.session_state:
    st.session_state.sheets_connection = initialize_sheets()

def log_to_sheets(word_data, selected_option, interaction_type, confidence, selection_count):
    """Log learning data to Google Sheets"""
    if st.session_state.sheets_connection:
        try:
            row_data = [
                datetime.now().isoformat(),
                word_data.get('clean', word_data.get('original', '').lower()),
            ...
            st.session_state.sheets_connection.append_row(row_data)
        except Exception as e:
...
def auto_learn_from_selection(word_data, selected_option, interaction_type="selection"):
    ...
    log_to_sheets(word_data, selected_option, interaction_type, 
                 auto_data[clean_word][selected_option]['confidence'],
                 auto_data[clean_word][selected_option]['count'])
...
with col1:
    st.title("🇦🇺 HCE IPA Transcriber with Google Sheets")
    st.markdown("*Auto-learning transcriber with cloud backup*")
...
with col2:
    with st.expander("⚙️ Settings"):
        st.session_state.auto_learn_enabled = st.checkbox(
            ...
            help="Automatically learn from your selections"
        )
        ...
        st.session_state.confidence_threshold = st.slider(
            ...
            step=0.1,
            help="Confidence needed to auto-promote to permanent dictionary"
        )
...
with st.sidebar:
    ...
    if st.session_state.sheets_connection:
        st.success("✅ Connected & Auto-syncing")
    else:
        st.error("❌ Not Connected")
        st.info("Add Google credentials for cloud backup")
...
with st.expander("⚙️ Google Sheets Setup"):
    ...
    **Quick Setup for Google Sheets Integration:**
    ...
    2. **Add Credentials:**
       ```
       # Save as google_credentials.json in your project folder
    ...
    4. **Done!** The app will automatically create a "HCE_IPA_Learning_Data" spreadsheet and start syncing your learning data.
    
    **Benefits:**
    - Your learning data is automatically backed up to Google Sheets
    - Access your learned words from any device
    - Never lose your pronunciation preferences
    """)

# --- Help Section ---
with st.expander("ℹ️ How to Use This App"):
    st.markdown("""
    **🎯 Simple Australian English IPA Transcriber with Smart Learning:**
    
    **How it Works:**
    1. **Type Text:** Enter any Australian English text
    2. **Choose Options:** Select the correct IPA pronunciation for each word
    3. **Auto-Learning:** The app learns your preferences automatically
    4. **Cloud Sync:** Your choices are saved to Google Sheets (if connected)
    
    **Learning Features:**
    - **Smart Confidence:** Manual corrections get higher confidence scores
    - **Auto-Promotion:** Frequently chosen options become permanent
    - **Google Sheets Sync:** Your learning data is backed up to the cloud
    - **Cross-Session Learning:** Your preferences persist across app sessions
    
    **Pro Tips:**
    - Use "Accept All & Learn" to quickly teach the app multiple words
    - Manual corrections (Custom field) build confidence faster
    - The app learns Australian pronunciation patterns over time
    - Green checkmarks (✅ Learned) show words the app has mastered
    """)